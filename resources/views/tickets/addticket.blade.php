            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{route('tickets.save',app()->getLocale())}}" method="POST"  >
							...
							<select class="form-control" name="user_id" @if(Auth::user()->is_owner() ) {{'readonly'}} @endif >
								@if(!empty($users))
									@foreach($users as $user)
										<option value="{{$user->id}}" @if(Auth::user()->is_owner() && Auth::user()->id == $user->id ) {{'selected'}} @elseif ($errors->any()&& old('user_id') == $user->id ) {{'selected'}} @elseif (Session::has('success')) {{''}}  @endif>{{$user->name}}</option>									
									@endforeach
								@endif
	                    ...
	                    <div class="form-group" style=" @if(Auth::user()->is_owner())  {{'display:none'}}  @endif ">
